...
// getmenu ให้ รายละเอีดร้านค้า
exports.menu  = async (req, res, next) => {
    // const menu = await Menu.find().select('+name -price');
    // const menu = await Menu.find();
    // const menu = await Menu.find().where('price').gt(10).sort('-_id');
    // const menu = await Menu.find({price:{$gt :10}});
    // populate is join in sql----------------------
...
// get shop by id ให้เมนูทั้งหมดมา
...
async function saveImageToGoogle(baseImage) {
    ...
    const storage = new Storage({
       projectId: 'kinetic-genre-311911',
       keyFilename: `${projectPath}/google_key.json`
    });

    const myBucket = storage.bucket('thejib_nodejs');
